...
class About extends Component {
  ...
  render() {
    const fontTest = () => {
      return ['none','plan1','plan2','plan3','plan4'].map((plan) => {
        return (
          <Button key={plan} onClick={()=>this.setFont(plan)} >{plan}</Button>
        )
      })
    }
    return (
      <div className={this.state.font === 'none' ? 'about' : this.state.font}>
        <div className='font-test'>{fontTest()}</div>
        ...
        <div className='aboutMain'>
          ...
          <p>Orchestra Mµsicart（オーケストラ・ミュージカルト）は2018年に「音楽環境つくり」「新制作への挑戦」「アート文化の発展」の三つの理念を掲げて結成されました。</p>
          <h5>
